...
> 原生窗口边框、视觉焕新，设置窗偏侧半透明与 toast 默认隐藏。

### 变更

- **原生窗口边框**：完整主窗与提示/确认/日志/更新/选择时间等对话框改为系统原生标题栏；Mini 仍保持无边框。内容区工具条保留版本号、NEW 角标、Mini、设置入口
- **视觉焕新**：按设计规范升级五主题色板（石板青蓝默认）、圆角/边距 Token、主窗工具条 ghost、倒计时主卡层次、设置 Tab 指示条；新增 `chip_active` / `toast_bg` 等语义别名
- **设置窗体验**：相对主窗/Mini 默认偏一侧并轻度透明；底部 toast 默认隐藏，有提示时再显示，避免空条遮挡

## 1.4.5

> Mini 左侧改为系统时钟与目标时间双行显示。

### 变更

- **Mini 左侧信息**：系统时钟与目标时间均显示 `HH:MM:SS`，双行布局；默认高度略增以容纳双行
...
- **启动自动班次**：设置 → 班次新增「启动时自动按班次倒计时」；需同时启用班次顺延
- **早于班次开始**：启动自动场景静默跳过，不弹窗

### 测试

- 配置读写、quiet 启动失败、`schedule_startup_shift` 调度相关单测

## 1.4.3

> 班次顺延倒计时、UI 样式体系重构（Token / Themed / 就地换肤 / 自定义色）、默认到期时间策略调整。

### 功能

- **班次顺延**：设置中配置计划开始/结束；`目标 = 结束 + (现在 − 开始)`；主界面「今日班次」、托盘/mac「按班次」
- **未启用班次**：主界面 chip 与托盘菜单项置灰（不再隐藏）
- **自定义主题色**：设置 → 外观可按语义键改色、预览、单键/全部重置并保存 `theme_custom`
- **到期时间默认**：启动与重置固定为 `18:00:00`（不再记忆上次输入）

### 重构 / 体验

- **设计 Token**：间距 / 圆角 / 字号贯通主窗、设置与对话框
- **小组件**：`section_title` / `divider` / `selectable_row` / `make_chip` 等去重设置页样板
- **Themed + 就地换肤**：主窗与设置窗主题切换优先 `recolor`，失败再回退重建
- **按钮语义**：ttk 主按钮与 `make_pill` / `make_chip` padding·色键对齐

### 测试

- 班次核心与控制器、Themed / 自定义色编辑、主题就地换肤等相关单测
